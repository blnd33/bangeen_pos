...
function saveExpense() {
  ...
  fetch('finance.php?lang=<?= LANG ?>', {method:'POST',body:fd})
    .then(r=>r.json())
    .then(d=>{
      if (d.success) { closeFinModal(); location.reload(); }
      else alert((LANG==='ar'?'خطأ: ':'Error: ')+(d.error||'Unknown'));
    })
    .catch(()=>alert('<?= LANG==="ar"?"حدث خطأ في الاتصال":"Connection error" ?>'));
}
function deleteExp(id) {
  ...
  fetch('finance.php?lang=<?= LANG ?>', {method:'POST',body:fd})
    ...
    .then(d=>{
      ...
      else alert('<?= LANG==="ar"?"فشل الحذف!":"Delete failed!" ?>');
    })
    .catch(()=>alert('<?= LANG==="ar"?"حدث خطأ في الاتصال":"Connection error" ?>'));
}
</script>
