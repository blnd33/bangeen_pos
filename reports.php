...
$cat_sales_stmt = $db->prepare("
    SELECT si.product_name_ar AS cat_ar, si.product_name_en AS cat_en,
           ...
           SUM(si.unit_price - si.subtotal) AS item_discount,
...
<div class="card" style="margin-top:1rem">
  ...
  <?php if (empty($cat_sales)): ?>
    <p style="color:var(--muted);font-size:.85rem;padding:.5rem 0">
      <?= LANG==='ar' ? 'لا توجد مبيعات في هذه الفترة' : 'No category POS sales in this period' ?>
    </p>
  <?php else: ?>
  <div class="table-wrap">
    <table>
      <thead>
        <tr>
          ...
          <th style="text-align:right"><?= LANG==='ar'?'مدخلات':'Entries' ?></th>
          <th style="text-align:right"><?= LANG==='ar'?'المبلغ الأصلي':'Gross' ?></th>
          <th style="text-align:right"><?= LANG==='ar'?'الخصم':'Discount' ?></th>
          <th style="text-align:right"><?= LANG==='ar'?'الصافي':'Net Sold' ?></th>
          <th style="text-align:right"><?= LANG==='ar'?'النسبة':'Share' ?></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          <td><strong><?= LANG==='ar' ? htmlspecialchars($cs['cat_ar']) : htmlspecialchars($cs['cat_en'] ?: $cs['cat_ar']) ?></strong></td>
          <td style="text-align:right;font-family:var(--font-en)"><?= number_format($cs['sale_rows']) ?></td>
          <td style="text-align:right;font-family:var(--font-en);color:var(--muted)"><?= number_format($cs['gross']) ?></td>
          <td style="text-align:right;font-family:var(--font-en);color:var(--danger)"><?= $cs['item_discount'] > 0 ? '-'.number_format($cs['item_discount']) : '\u2014' ?></td>
          <td style="text-align:right;font-family:var(--font-en);font-weight:700;color:var(--brand)"><?= number_format($cs['net']) ?></td>
          <td style="text-align:right">
            <div style="display:flex;align-items:center;justify-content:flex-end;gap:.4rem">
              <div style="width:55px;height:6px;background:var(--border);border-radius:3px;overflow:hidden">
                <div style="width:<?= $share ?>%;height:100%;background:var(--brand);border-radius:3px"></div>
              </div>
              <span style="font-size:.78rem;font-family:var(--font-en);color:var(--muted)"><?= $share ?>%</span>
            </div>
          </td>
        </tr>
        <?php endforeach; ?>
      ...
      <tfoot>
        <tr style="background:var(--surface2)">
          ...
          <td style="text-align:right;font-family:var(--font-en)"><strong><?= number_format(array_sum(array_column($cat_sales,'sale_rows'))) ?></strong></td>
          <td style="text-align:right;font-family:var(--font-en);color:var(--muted)"><strong><?= number_format($cat_gross_total) ?></strong></td>
          <td style="text-align:right;font-family:var(--font-en);color:var(--danger)"><strong><?= $cat_discount_total > 0 ? '-'.number_format($cat_discount_total) : '\u2014' ?></strong></td>
          <td style="text-align:right;font-family:var(--font-en);font-weight:900;color:var(--brand);font-size:1.05rem"><?= number_format($cat_net_total) ?> <span style="font-size:.75rem"><?= $currency ?? 'IQD' ?></span></td>
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.75rem;margin-top:1rem">
    <div style="background:var(--surface2);border-radius:10px;padding:.75rem;text-align:center">
      <div style="font-size:.7rem;color:var(--muted);margin-bottom:.25rem"><?= LANG==='ar'?'إجمالي المبيعات':'Total Sold' ?></div>
      <div style="font-size:1.2rem;font-weight:800;color:var(--brand);font-family:var(--font-en)"><?= number_format($cat_net_total) ?></div>
      <div style="font-size:.7rem;color:var(--muted)"><?= $currency ?? 'IQD' ?></div>
    </div>
    <div style="background:var(--surface2);border-radius:10px;padding:.75rem;text-align:center">
      <div style="font-size:.7rem;color:var(--muted);margin-bottom:.25rem"><?= LANG==='ar'?'إجمالي الخصومات':'Total Discounts' ?></div>
      <div style="font-size:1.2rem;font-weight:800;color:var(--danger);font-family:var(--font-en)"><?= number_format($cat_discount_total) ?></div>
      <div style="font-size:.7rem;color:var(--muted)"><?= $currency ?? 'IQD' ?></div>
    </div>
    <div style="background:var(--surface2);border-radius:10px;padding:.75rem;text-align:center">
      <div style="font-size:.7rem;color:var(--muted);margin-bottom:.25rem"><?= LANG==='ar'?'الفئات النشطة':'Active Categories' ?></div>
      <div style="font-size:1.2rem;font-weight:800;color:var(--text);font-family:var(--font-en)"><?= count($cat_sales) ?></div>
    </div>
  </div>
  <?php endif; ?>
</div>
